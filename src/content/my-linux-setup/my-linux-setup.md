...
使い方はごくシンプルで`awww-daemon`を起動した後に、`awww img <path/to/img>`で壁紙が適用されます。しかし`awww`にはスライドショー的な機能はなく前まではbash scriptでどうにかしていました。

ただスライド用のディレクトリの切り替えなどの機能が欲しくなって来て、bashで書くのが限界になったため、Rustで同様のツールを書き直しました。[wlmstr](https://github.com/Uliboooo/wlmstr)

やってることはごく単純で、現在のフォルダパス、現在の背景画像パスを`XDG_DATA_HOME/wlmstr/data.json`に保持して、適切な`awww`を実行するというラッパーです。
